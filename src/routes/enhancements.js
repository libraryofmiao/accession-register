...
export function createEnhancementRouter(supabaseAdmin){
  ...
  // Return the complete DDC master for the authenticated DDC Master page.
  ...
  router.get("/ddc/lookup",async(req,res)=>{const q=esc(req.query.q),p=`%${q}%`;if(!q)return res.json([]);let z=supabaseAdmin.from("ddc_master").select("id,ddc_number,subject,division_number,section_number").order("ddc_number").limit(20);z=z.or(`ddc_number.ilike.${p},subject.ilike.${p}`);const {data,error}=await z;if(error)return res.status(500).json({error:error.message});res.json(data||[]);});
