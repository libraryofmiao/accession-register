...
async function loadDdc(){
  ...
  try{
    const url=search?'/api/admin/ddc?q='+encodeURIComponent(search):'/api/admin/ddc';
    const r=await fetch(url);
    if(!r.ok) throw new Error('Unable to load DDC records.');
...
(async()=>{if(await ensure()){await loadDdc();await loadLoc()}})();
